...
function patchElement(oldVdom, newVdom, hostComponent) {
  const { el, listeners: oldListeners } = oldVdom;
  const {
    class: oldClass,
    style: oldStyle,
    on: oldEvents,
  ...
  for (const operation of diffSeq) {
    ...
    switch (operation.op) {
      ...
      case ARRAY_DIFF_OP.MOVE:
        ...
        const newChild = newChildren[index + offset];
      ...
      case ARRAY_DIFF_OP.NOOP:
        ...
        break;
      default:
        throw new Error(`Unknown array diff operation ${operation.op}`);
    }
  }
}
